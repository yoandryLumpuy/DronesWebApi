...
namespace DronesWebApi.UnitTests.Validators.Medication.Commands.CreateMedicationCommand
{
    ...
    {
        public CreateMedicationCommandValidatorTester(IUnitOfWork unitOfWork)
        {
            //_unitOfWork = unitOfWork;

            //RuleFor(m => m.Name)
            //    .Must(name => Regex.IsMatch(input: name, pattern: @"^[a-zA-Z\\d\\-_]+$"))
            //    .WithMessage($"{nameof(Models.Medication.Commands.CreateMedicationCommand.CreateMedicationCommand.Name)} field only accepts: letters, numbers, '-' and '_' characters");

            //RuleFor(m => m.Code)
            //    .Must(code => Regex.IsMatch(input: code, pattern: @"^[A-Z\\d_]+$"))
            //    .WithMessage($"{nameof(Models.Medication.Commands.CreateMedicationCommand.CreateMedicationCommand.Code)} only accepts: upper case letters, underscore and numbers");
